...
function fnSave() {
	...
	if(document.detailForm.mode.value == "insert") {
		document.detailForm.action = "<c:url value='/pattern/mp31/insertEmployee.do'/>";
	}else{
		document.detailForm.action = "<c:url value='/pattern/mp31/updateEmployee.do'/>";
	}
	document.detailForm.submit();
}
function fnRetrieve() {
	document.detailForm.action = "<c:url value='/pattern/mp31/retrieveEmployee.do'/>";
	document.detailForm.submit();
}
function fnRetrieveList() {
	document.detailForm.action = "<c:url value='/pattern/mp31/retrieveEmployeeList.do'/>";
	document.detailForm.submit();
}
						...
						<tr>
							...
							<td>
								<c:choose>
									...
									<c:when test="${param.mode == 'update'}">
										<input type="hidden" class="Ltext" id="num" name="num" value="${result.num}" /><c:out value="${result.num}" />
									</c:when>
								</c:choose>
							</td>
