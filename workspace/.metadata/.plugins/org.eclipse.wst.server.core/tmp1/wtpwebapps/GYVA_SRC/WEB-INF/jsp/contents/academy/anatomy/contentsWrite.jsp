...
//<![CDATA[
	...
	function checkValue() {
		...
		if (!document.anatomyContentsInfo.title.value) {
 		...
 		if (!document.anatomyContentsInfo.uploadFileThum.value) {
 			alert("썸네일 이미지를 등록하세요.");
			return false;
		}
		
		if (!document.anatomyContentsInfo.uploadFileDetail.value) {
	...
	function goAnatomyContentsList() {
		document.anatomyContentsInfo.action = "<c:url value='/contents.do'/>";
		document.anatomyContentsInfo.submit();
	}
				...
				<table class="default-table">
					...
					<tbody>					
							...
							<td>					     		
								<ul class="form" data-form="radio">
									...
									<c:set var="index" value="${status.index}" />								
										<li>
											<input type="radio" class="check-form" name="categoryCode" id="menuId_<c:out value='${index}'/>" value="<c:out value='${categoryCode.code}'/>"
												<c:if test="${input.categoryCode == categoryCode.code}">checked</c:if> />
						...
						<tr>
							...
							<td>
								<div class="form"><textarea class="input-form" name="contents" id="contents" placeholder="강의 내용을 입력해주세요." rows="12"></textarea></div>
							</td>
						</tr>
