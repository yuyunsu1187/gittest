...
function bodyControll(){	
	return{
		...
		movingItem : [],
		movingOption : {
			thumbnail : app.$body.find('.list-wrap'),
			increase :10,
		},
		moving : function(){
			var _window = $(window);
			...
			var _option = this.option;
			var _movingOption = this.movingOption;
			var _loadingArea = app.$body.find('.loading-area');
			var _count;
			var _list;
			var _length;
			var _height;
			...
			/* 01. load */
			if(!hasJqueryObject(app.$body.find('[data-type="swiper"]')) && !hasJqueryObject(app.$body.find('#FR010501-05')) && hasJqueryObject(app.$body.find('.list-wrap'))) app.bodyControll.thumbnailView(app.bodyControll.movingOption.thumbnail, app.bodyControll.movingOption.increase);
			
			/* 02. click */
			_document.on("click", '.gotop-btn', function(){app.bodyControll.topMoving(_document, 0);}); // 04. 최상단으로 스크롤 이동
		
			/* 03. scroll */
			_window.on({
				scroll : function(e){
					var _windowScroll = $(this).scrollTop(); // 현재 스크롤 죄표 값
					_count = app.bodyControll.movingOption.increase;
					_list = app.bodyControll.movingOption.thumbnail.find(".thumbnail-list li");
					...
					if(!hasJqueryObject(app.$body.find('[data-type="swiper"]')) && !hasJqueryObject(app.$body.find('#FR010501-05')) && hasJqueryObject(app.$body.find('.list-wrap'))){
						if(_length > _count || !hasJqueryObject(app.$body.find('[data-tab-action="hide"]'))){
							if(_windowScroll > _maxScroll){
								app.bodyControll.movingOption.increase = _count + _count;
								_count = app.bodyControll.movingOption.increase;
								
								_loadingArea.attr("data-pop-action", "active");
								setTimeout(function(){
									_loadingArea.attr("data-pop-action", "disabled");
									for(var i=0;i<_count;i++){_list.eq(i).show();}
								}, 1000);
							}
							_maxScroll = parseInt(_height) * _count;
						}
					}
				}
			});			
		},
		thumbnailView : function(_thumbnail, _count){
			var _list = _thumbnail.find(".thumbnail-list li");
...
function formCotroll(){
	return{
		...
		radioCheck : function(_this){
			...
			if(!app.$body.find(".article-tit + .form-area")) $('[data-position-key="' + _key +'"]').find(".input-form, .select-form, .select-btn").attr("required", "required");
		...
		optionList : function(_this, _selectBtn, _area, _wrap, _option, _item, _offset){
			...
			if(_this.hasClass("option-btn")){
				...
				//_selectBtn.context.firstChild.data = _data; // 퍼블리싱 페이지 옵션값 추출적용, n.fn.init[]
				//console.log(_selectBtn[0].innerText, _selectBtn.get(), _selectBtn);
...
function tabControll(){		
	return{
		...
		init : function(){
			...
			var _tabList = _target.find('[data-' + _option.tab + '-area="' + _option.tab + '-' + _option.list + '"]'); // 모바일 전용 탭 리스트
			...
			if(_target.attr("data-tab-division") == "article") _target.on("click", ".option-list .list-btn, .option-list .option-btn", function(){app.tabControll.click(_option, _list, _cont, $(this));}); // 클릭 액션
		},
		...
		click : function(_option, _list, _cont, _this){
			...
			app.bodyControll.movingOption = {thumbnail : _cont.eq(_index).find('.list-wrap'), increase : 10};
			app.bodyControll.thumbnailView(app.bodyControll.movingOption.thumbnail, app.bodyControll.movingOption.increase);
		},
	}
}
	...
	return{
		...
		init : function(){
			...
			if(hasJqueryObject(app.$body.find(_toggle))) {
				...
				/* 커튼 레이어 */
				$("." + _option.curtain).on("click", ".search-btn", function(){app.toggleControll.curtain($(this));});
			}
		},
		...
		curtain : function(_this){
			var _target = _this.next();
			(!_this.hasClass("active")) ? (_this.addClass("active"), _target.slideDown()) : (_this.removeClass("active"), _target.slideUp());
		},
	}
}
...
function swiperControll(){
	return{
		...
		init : function(){
			...
			_target.on("click", ".autoplay-btn", function(){app.swiperControll.autoplay($(this), _option);}); // 메인 비주얼 영역
		},
		swiper : function(_target, _option){
			...
			_target.each(function(i){
				...
				if(_data == "scrollbar") _setting = _option.scrollbar;
				if(_data == "visual") _setting = _option.visual; // 메인 상단 비주얼
				// 메인 상단 비주얼
				if(_data == "visual"){
				...
				if(_data == "fraction") _setting = _option.fraction; // 이벤트 페이지
...
function closeCotroll(){
	return{
		...
		closePop : function(_this){
			...
			if(_this.parents(".layerPop-area").attr("data-pop-type") == "terms") {
				var _check = _this.data("check-key");
				var _terms = app.$body.find('[data-btn-key="' + _check + '"]');
				
				_terms.addClass("active");
			}
