...
function fnRetrieveList() {
	document.searchForm.action = "<c:url value='/admin/contents/injection/retrieveContentsList.do'/>";
...
function fnlikeDislike(listForm, seq, contentsId) {
	...
	$.ajax({
		url : "/admin/contents/injection/updateLikeDislike.do",
		type : "post",
		dataType : "json",
...
function fnDetail(contentsId, categoryCode, mode) {
	...
	listForm.action = "<c:url value='/admin/contents/injection/retrieveContents.do'/>";
	listForm.submit();
...
function fnUpdate() {
	listForm.action = "<c:url value='/admin/contents/academy/injection/contentsUpdate.do'/>";
	listForm.submit();
}

function fnDelete() {
	
	var delConfirm = confirm('해당 관리자를 삭제하시겠습니까?');
	if (delConfirm) {
		alert('삭제되었습니다.');
		listForm.action = "<c:url value='/admin/contents/academy/injection/deleteContents.do'/>";
		listForm.submit();
	} 
}
...
function goInjectionContentsList() {
	listForm.action = "<c:url value='/admin/contents/injection/retrieveContentsList.do'/>";
	listForm.submit();
}

//취소 클릭시Injection 컨텐츠 화면으로
...
<div id="LblockBody">
	<div id="LblockPageHeader">
		...
		<div id="LblockPageLocation">
			<ul>
				<li class="Llast"><span>YVOIRE Injection Detail</span></li>
			</ul>
		</div>
	</div>
	...
	<div id="LblockBodyMain">
		...
		<div id="LblockListTable01" class="LblockListTable" >
			...
			<div class="Lwrapper">
			<b><font size="4" color="gray">YVOIRE Injection Detail</font></b>
	...
	<!--	<button type="button" name="next" id="next"  value="next" onclick="javascript:fnDetail(${result.contentsId}, ${result.categoryCode}, document.getElementById('next').value)">Next</button>  -->
		...
		<input type="button" value="List" onclick="goInjectionContentsList()" />
