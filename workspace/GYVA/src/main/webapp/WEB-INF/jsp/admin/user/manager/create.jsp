	...
	<article class="board-wrap" data-board-type="form-write">
		<div class="board-area">
			...
			<form method="post" action="/admin/user/manager/insertManager.do" name="userInfo" onsubmit="return checkValue()">
				...
				<input type="hidden" name="divisionCode" id="divisionCode" value="00"/>
				<input type="hidden" name="countryCode" id="countryCode" value="00"/>
				...
				<table class="default-table">					
					...
					<tbody>				
						<tr>
							...
							<td>
								...
								<c:forEach items = "${codeDetailList}" var = "codeDetail">
									<option value="<c:out value='${codeDetail.code}'/>"> <c:out value="${codeDetail.value}"/> </option>
								</c:forEach>
								</select>
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<div class="form-area" data-combine-form="column-2-form">
									...
									<div class="form" data-mult-form="password">
										<input type="password" class="input-form" name="passwordCheck" id="passwordCheck" maxlength="50" placeholder="비밀번호를 다시 입력해주세요." onclick="fnPwdChk()" />
										<button type="button" class="icon-btn"><i class="far fa-eye"></i></button>
									</div>
						...
						<tr>
							<th scale="col">담당</th>
							<td><div class="form"><input type="text" class="input-form" placeholder="담당을 입력해주세요." /></div></td>
						</tr>
					</tbody>	
				</table>
			</form>
