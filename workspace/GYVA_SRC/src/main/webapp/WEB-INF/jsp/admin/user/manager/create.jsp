...
/*             if(!document.userInfo.nameLast.value){
    	...
    	// 비밀번호 on/off
    	function toggleEye(num) {
    		$('.form-area .form:nth-child(' + num +')').toggleClass('active');
    		if($('.form-area .form:nth-child(' + num +')').hasClass('active')){
	        	$('.form-area div:nth-child(' + num + ') i').attr('class',"fa fa-eye-slash");
	            $('.form-area div:nth-child(' + num + ') input').attr('type',"text");
	        }else{
	            $('.form-area div:nth-child(' + num + ') i').attr('class',"fa fa-eye");
	            $('.form-area div:nth-child(' + num + ') input').attr('type','password');
	        }
...
<section class="content-section">
	...
	<article class="board-wrap" data-board-type="form-write">
		...
		<input type="hidden" name="createId" id="createId" value="<c:out value='${userId }'/>"/>
		<input type="hidden" name="confirmId" id="confirmId" value="<c:out value='${userId }'/>"/>
						...
						<tr>
							...
							<td>
								...
								<c:set var="index" value="${status.index}" />
									<li>
										<input type="radio" class="check-form" id="radio_<c:out value='${index}'/>" name="admin" value="<c:out value='${userType.code}'/>" <c:if test="${index == 0}">checked</c:if>/>
							...
							<td>
								<div class="form-area" data-combine-form="column-2-form">
									<div class="form" data-mult-form="password">
										<input type="password" class="input-form" name="password" id="password" data-validate-key="userPassword" placeholder="비밀번호를 입력해주세요." />
										<!-- 비밀번호 보기 : <i class="fas fa-eye"></i> -->
										<button type="button" class="icon-btn" ><i class="far fa-eye"></i></button>
									</div>
									
									<div class="form" data-mult-form="password">
										<input type="password" class="input-form" name="passwordCheck" id="passwordCheck" data-validate-key="userPassword" placeholder="비밀번호를 다시 입력해주세요."/>
										<button type="button" class="icon-btn" ><i class="far fa-eye"></i></button>
									</div>
