...
<script type="text/javaScript" language="javascript">
	...
	function fnDetail(value) {
	    document.listForm.groupId.value = value;
		document.listForm.action = "<c:url value='/admin/system/groupCodeDetail.do'/>";
		document.listForm.submit();
	}
	
	// 글 작성 화면으로 이동
	function fnWrite() {
		document.listForm.action = "<c:url value='/admin/system/groupCodeForm.do'/>";
		document.listForm.submit();
	}
	// ]]>
</script>
			
<section id="AD070101-05" class="content-section">
				...
				<table class="default-table">
					...
					<tbody>
						<c:forEach var="result" items="${resultList}" varStatus="status">
							<tr>
								<td data-text-style="ellipsis"><a href="javascript:fnDetail('<c:out value="${result.groupId}"/>')"><c:out value="${result.groupName}" /></a></td>
						...
						<c:if test="${empty resultList && !empty input}">
							<tr>
								<td colspan="4" data-td-type="board-noData">등록된 그룹코드가 없습니다</td>
							</tr>
						</c:if>
					</tbody>
				</table>
