...
//<![CDATA[
	...
	function checkValue() {
		...
		if (!document.injectionContentsInfo.title.value) {
			alert("제목을 입력하세요.");
			return false;
		}
		...
		if (!document.injectionContentsInfo.readerUserId.value) {
			alert("담당 강사를 선택 하세요.");
			return false;
		}
...
function goInjectionContentsList() {
	document.injectionContentsInfo.action = "<c:url value='/contents.do'/>";
	document.injectionContentsInfo.submit();
}
