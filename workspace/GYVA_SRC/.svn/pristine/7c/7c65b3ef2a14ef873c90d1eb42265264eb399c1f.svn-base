package devonframe.gyva.admin.contents.survey.excel.view;
...
public class ExcelDownloadView extends AbstractView {
    ...
    protected void setResponseHeadersForMulitBrowser(HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException {
        ...
        if (sUserAgent.indexOf("MSIE 5.5") != -1) {
            response.setHeader(CONTENT_DISPOSITION, "attachment; filename=\"" + FileUtil.encodeURLEncoding(fileAlias, encoding).replace("\r", "").replace("\n", "") + "\";");
        } else {
            ...
            if (isChromeOrFirefox) {
                response.setHeader(CONTENT_DISPOSITION, openStyle + "; filename=" + "\"=?" + this.encoding + "?Q?" + FileUtil.encodeQuotedPrintable(fileAlias, this.encoding).replace("\r", "").replace("\n", "") + "?=\";");
            } else {
                response.setHeader(CONTENT_DISPOSITION, openStyle + "; filename=\"" + FileUtil.encodeURLEncoding(fileAlias, encoding).replaceAll("\\+", " ").replace("\r", "").replace("\n", "") + "\"");
            }
        }
    }
