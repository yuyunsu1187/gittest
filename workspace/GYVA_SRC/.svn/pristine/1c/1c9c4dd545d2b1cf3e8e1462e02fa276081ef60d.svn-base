...
<section id="AD050303-20" class="content-section">
				...
				<table class="default-table">
					...
					<tbody>
						<tr>
							...
							<td>
								<div class="form"><input type="text" class="input-form" name="title" id="title" maxlength="50" value="<c:out value='${contentsResult.title}'/>" /></div>
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<div class="form-area" data-combine-form="column-2-form">
									<div class="form"><input type="text" class="input-form" name="keyword1" id="keyword1" maxlength="255" value="<c:out value='${contentsResult.keyword1}'/>" placeholder ="Enter keyword 1"></div>
									<div class="form"><input type="text" class="input-form" name="keyword2" id="keyword2" maxlength="255" value="<c:out value='${contentsResult.keyword2}'/>" placeholder ="Enter keyword 2"></div>
								</div>
							</td>
						</tr>
						<tr>
							...
							<td>
								<div class="form"><textarea class="input-form" name="contents" id="contents" placeholder="내용을 입력해주세요." rows="12"><c:out value='${contentsResult.contents}'/></textarea></div>
							</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td>
								...
								<ul class="form" data-form="radio">
									<li>
										<input type="radio" class="check-form" id="contentsVideo" name="setting" data-radio-key="video" <c:if test="${fn:length(fileInfoList)==2}">checked</c:if> disabled/>
										<label for="contentsVideo" class="label-form">동영상</label>
									</li>
									<li>
										<input type="radio" class="check-form" id="contentsImage" name="setting" data-radio-key="image" <c:if test="${fn:length(fileInfoList)>2}">checked</c:if> disabled/>
						...
						<tr data-radio-target="position" data-position-key="video">
							...
							<td>
								<input type=hidden name="fileName" <c:if test="${empty contentsResult.sldFileName }">value="<c:out value='${contentsResult.fileName}'/>"</c:if> />
								<input type="hidden" name="uploadFilePath" <c:if test="${empty contentsResult.sldFileName }">value="<c:out value='${contentsResult.filePath}'/>"</c:if> />
								<div class="form" data-mult-form="uploade-file">
						...
						<th scale="col" data-text-style="essential">다운로드 파일 등록</th>
							<td>
								<input type="hidden" name="fileName" <c:if test="${not empty contentsResult.sldFileName }">value="<c:out value='${contentsResult.fileName}'/>"</c:if> />
								<input type="hidden" name="uploadFilePath" <c:if test="${not empty contentsResult.sldFileName }">value="<c:out value='${contentsResult.filePath}'/>"</c:if> />
								<div class="form" data-mult-form="uploade-file">
									<a href="#" class="link-btn" onclick="fnDownload(1);"><c:out value='${contentsResult.fileName}'/></a>
								</div>
								<div class="form" data-mult-form="file">
