...
<script type="text/javascript" language="javascript">
	...
	function fnQuestionSave() {
		document.listForm.action = "<c:url value='/contents/anatomy/insertQuestionInfo.do'/>";
		document.listForm.submit();
	}
	...
	function fnAnswerSave(seq, type) {
		...
		document.listForm.action = "<c:url value='/contents/anatomy/updateQuestionInfo.do'/>";
		document.listForm.submit();	
	}
	function fnMoveContents(type) {
		...
		document.listForm.action = "<c:url value='/contents/anatomyDetail.do'/>";
		document.listForm.submit();
	}
	function fnSurveySave() {
		if(document.surveyForm.answer1.value == '') {
			alert("Please enter the 1 answers");
			$("#answer1").focus();
			return false;	
		} else if(document.surveyForm.answer2.value == '') {
			alert("Please enter the 2 answers");
			$("#answer2").focus();
			return false;		
		} else if(document.surveyForm.answer3.value == '') {
			alert("Please enter the 3 answers");
			$("#answer3").focus();
			return false;		
		} else if(document.surveyForm.answer4.value == '') {
			alert("Please enter the 4 answers");
			$("#answer4").focus();
			return false;		
		} else if(document.surveyForm.answer5.value == '') {
			alert("Please enter the 5 answers");
			$("#answer5").focus();
			return false;		
		} else {
			document.surveyForm.action = "<c:url value='/contents/anatomy/insertSurveyResult.do'/>";
	...
	function goUpdateContents() {
		document.listForm.action = "<c:url value='/contents/anatomyEdit.do'/>";
		document.listForm.submit();
	}
	...
	function fnDeleteContents() {
		...
		if (delConfirm) {
			document.listForm.action = "<c:url value='/contents/anatomyDelete.do'/>";
			document.listForm.submit();
		}
	}
...
<section id="FR070101-10" class="content-section">
	...
	<!-- [2020-08-28] data-board-header="download" data- 속성값 추가 -->
							...
							<div class="comment-wrap" data-comment-type="certified-instructor">
								...
								<c:if test="${helpdesk.stateCode=='02'}">
									<article class="comment-contents" data-comment-condition="reply">
										<header class="comment-header">
											<i class="profile-icon"><img src="<c:out value='${helpdesk.lecturerImgPath}'/>" alt="" /></i>
		...
		<div class="submit-area">
			...
			<c:if test="${menuType=='AD'}">
				<button type="submit" class="sub-btn" data-btn-key="poll" onclick="fnRetrieveList();"><i class="fas fa-list left"></i>List</button>
...
<div class="layerPop-wrap" id="FR070101-15" data-pop-action="disabled">
...
<div class="layerPop-wrap" id="FR070101-20" data-pop-action="disabled">
