package devonframe.gyva.common.ui;
...
public class ContentsPagingUI implements PagingUI{
    ...
    public String showMoveFirstPage() {
        if (getIndexes() > 0 && pagingList.getCurrentPage() > 1) {
            ...
            sb.append("<button type=\"button\" class=\"paging-btn\" onclick=\"goPage('")
              .append(1)
              .append("'); return false;\" >")
    ...
    public String showMoveBeforePage() {
        ...
        if (getIndexes() > 0 && targetPage > 0) {
            String titleAttr = "이전 페이지로 이동";
            StringBuffer sb = new StringBuffer();
            sb.append("<button type=\"button\" class=\"paging-btn\" onclick=\"goPage('")
              .append(getTargetRow(targetPage))
              .append("'); return false;\" >")
              .append(getImageTag(pagingUIConfig.getMoveBeforePage(), titleAttr, targetPage +"page"))
              .append("</button>");    
            return sb.toString();            
        }
        return "";
    ...
    public String showMoveNextPage() {
        ...
        if (getIndexes() > 0 && targetPage <= getPages()) {
            String titleAttr = "다음 페이지로 이동";
            StringBuffer sb = new StringBuffer();
            sb.append("<button type=\"button\" class=\"paging-btn\" onclick=\"goPage('")
              .append(getTargetRow(targetPage))
              .append("'); return false;\" >")
              .append(getImageTag(pagingUIConfig.getMoveNextPage(), titleAttr, targetPage +"page"))
              .append("</button>");    
            return sb.toString();
        }
        return "";
    ...
    public String showIndex() {
        ...
        for (int targetPage = startPageIndex; targetPage < endPageIndex; targetPage++) {
            ...
            String showIndexStr = "";
            if (targetPage == currentPage) {
                showIndexStr = "<strong>" + targetPage + "</strong>";
            } else {
                StringBuffer sb = new StringBuffer();
                sb.append("<button type=\"button\" class=\"paging-btn\" onclick=\"goPage('")
                  .append(getTargetRow(targetPage))
